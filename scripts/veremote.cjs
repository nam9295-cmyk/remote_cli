...
function printHelp() {
  ...
  console.log(paint("─────", ANSI.line));
  console.log(`${paint("init".padEnd(15), ANSI.bold)} Run the interactive setup wizard for .env.local`);
...
function getEnvTemplate() {
  return buildEnvFileContent({
    ...
    telegramPollingIntervalMs: "1000",
    publicBaseUrl: "",
    workspacePreviewUrl: "http://127.0.0.1:5173",
    workspacePreviewImagePath: "",
    veremoteAllowedPreviewRoots: "",
...
function buildEnvFileContent(config) {
  return [
    ...
    "# Generated by `veremote init`. Update values here later if needed.",
    "",
    "TELEGRAM_BOT_TOKEN=" + (config.telegramBotToken || ""),
    "TELEGRAM_CHAT_ID=" + (config.telegramChatId || ""),
    "TELEGRAM_POLLING_ENABLED=" + (config.telegramPollingEnabled || "true"),
    "TELEGRAM_POLLING_INTERVAL_MS=" + (config.telegramPollingIntervalMs || "1000"),
    "",
    "# Optional public link base for Telegram detail links",
    "PUBLIC_BASE_URL=" + (config.publicBaseUrl || ""),
    "",
    "# Preview defaults for new workspaces",
    "WORKSPACE_PREVIEW_URL=" + (config.workspacePreviewUrl || ""),
    "# WORKSPACE_PREVIEW_IMAGE_PATH=" + (config.workspacePreviewImagePath || ""),
    "# VEREMOTE_ALLOWED_PREVIEW_ROOTS=" + (config.veremoteAllowedPreviewRoots || ""),
    "",
    "# Engine commands",
    "GEMINI_CLI_COMMAND=" + (config.geminiCliCommand || ""),
    "CODEX_CLI_COMMAND=" + (config.codexCliCommand || ""),
    "# CUSTOM_CLI_COMMAND=" + (config.customCliCommand || ""),
    "",
  ].join("\n");
}
...
function isInteractiveInitAvailable() {
...
async function promptWithDefault(rl, label, defaultValue, options = {}) {
  const displayedDefault = options.maskDefault ? maskSecret(defaultValue) : defaultValue;
...
async function promptChoice(rl, label, choices, defaultValue) {
  ...
  for (const choice of choices) {
    const hint = choice.detail ? ` - ${choice.detail}` : "";
    console.log(`  ${choice.key}) ${choice.label}${hint}`);
  }

  const answer = (
    await rl.question(`Select${defaultValue ? ` [${defaultValue}]` : ""}: `)
  )
    .trim()
    .toLowerCase();
  ...
  const matched = choices.find(
    (choice) =>
      choice.key.toLowerCase() === answer ||
      choice.label.toLowerCase() === answer,
  );
...
function getInitDefaults() {
  ...
  const geminiResolved = getCommandPath("gemini");
  const codexResolved = getCommandPath("codex");

  return {
    envExists: fs.existsSync(ENV_PATH),
    telegramBotToken:
      current.TELEGRAM_BOT_TOKEN || process.env.TELEGRAM_BOT_TOKEN || "",
    telegramChatId:
      current.TELEGRAM_CHAT_ID || process.env.TELEGRAM_CHAT_ID || "",
    telegramPollingEnabled:
      current.TELEGRAM_POLLING_ENABLED || process.env.TELEGRAM_POLLING_ENABLED || "true",
    telegramPollingIntervalMs:
      current.TELEGRAM_POLLING_INTERVAL_MS ||
      process.env.TELEGRAM_POLLING_INTERVAL_MS ||
      "1000",
    publicBaseUrl: current.PUBLIC_BASE_URL || process.env.PUBLIC_BASE_URL || "",
    workspacePreviewUrl:
      current.WORKSPACE_PREVIEW_URL ||
      process.env.WORKSPACE_PREVIEW_URL ||
      "http://127.0.0.1:5173",
    workspacePreviewImagePath:
      current.WORKSPACE_PREVIEW_IMAGE_PATH ||
      process.env.WORKSPACE_PREVIEW_IMAGE_PATH ||
      "",
    veremoteAllowedPreviewRoots:
      current.VEREMOTE_ALLOWED_PREVIEW_ROOTS ||
      process.env.VEREMOTE_ALLOWED_PREVIEW_ROOTS ||
      "",
    geminiCliCommand:
      current.GEMINI_CLI_COMMAND ||
      process.env.GEMINI_CLI_COMMAND ||
      (geminiResolved ? "gemini" : ""),
    codexCliCommand:
      current.CODEX_CLI_COMMAND ||
      process.env.CODEX_CLI_COMMAND ||
      (codexResolved ? "codex" : ""),
    customCliCommand:
      current.CUSTOM_CLI_COMMAND || process.env.CUSTOM_CLI_COMMAND || "",
    geminiResolved,
    codexResolved,
  };
}
...
async function runInteractiveInit() {
  ...
  try {
    console.log(paint("veremote setup", ANSI.label));
    console.log(paint("──────────────", ANSI.line));
    console.log("Press Enter to keep the suggested value.");
    console.log("");

    const telegramBotToken = await promptWithDefault(
      rl,
      "Telegram bot token",
      defaults.telegramBotToken,
      { required: true, maskDefault: true },
    );
    const telegramChatId = await promptWithDefault(
      rl,
      "Telegram chat id",
    ...
    const engineChoice = await promptChoice(
      rl,
      ...
      [
        {
          key: "gemini",
          label: "gemini",
          detail: defaults.geminiResolved
            ? `detected at ${defaults.geminiResolved}`
            : "not detected, but you can still configure it",
        },
        {
          key: "codex",
          label: "codex",
          detail: defaults.codexResolved
            ? `detected at ${defaults.codexResolved}`
            : "not detected, but you can still configure it",
        },
        {
          key: "both",
          label: "both",
          detail: "configure gemini and codex together",
        },
      ],
      defaults.codexCliCommand && defaults.geminiCliCommand ? "both" : defaults.geminiCliCommand ? "gemini" : "codex",
    );

    let geminiCliCommand = defaults.geminiCliCommand;
    let codexCliCommand = defaults.codexCliCommand;

    if (engineChoice === "gemini") {
      geminiCliCommand = await promptWithDefault(
        rl,
        "Command to run Gemini",
        defaults.geminiCliCommand || "gemini",
        { required: true },
      );
    }

    if (engineChoice === "codex") {
      codexCliCommand = await promptWithDefault(
        rl,
        "Command to run Codex",
        defaults.codexCliCommand || "codex",
        { required: true },
      );
    }
    ...
    const previewChoice = await promptChoice(
      rl,
      "Default preview mode for new workspaces",
      [
        {
          key: "web",
          label: "web",
          detail: "use a localhost URL for real browser screenshots",
        },
        {
          key: "image",
          label: "image",
      ...
      defaults.workspacePreviewImagePath
        ? "image"
        : defaults.workspacePreviewUrl
          ? "web"
          : "none",
    );

    let workspacePreviewUrl = "";
    let workspacePreviewImagePath = "";

    if (previewChoice === "web") {
      workspacePreviewUrl = await promptWithDefault(
        rl,
        "Preview URL",
        defaults.workspacePreviewUrl || "http://127.0.0.1:5173",
      );
    }
    ...
    const publicBaseUrl = await promptWithDefault(
      rl,
      "Public base URL for Telegram detail links",
    ...
    const config = {
      telegramBotToken,
      telegramChatId,
      telegramPollingEnabled: defaults.telegramPollingEnabled || "true",
      telegramPollingIntervalMs: defaults.telegramPollingIntervalMs || "1000",
      publicBaseUrl,
      workspacePreviewUrl,
      workspacePreviewImagePath,
      veremoteAllowedPreviewRoots: defaults.veremoteAllowedPreviewRoots || "",
      geminiCliCommand,
      codexCliCommand,
      customCliCommand: defaults.customCliCommand || "",
    };

    fs.writeFileSync(ENV_PATH, buildEnvFileContent(config), "utf8");
    applyEnvConfig(config);

    printMessage(`${ENV_PATH} updated.`);
    console.log(paint("Summary", ANSI.label));
    console.log(paint("───────", ANSI.line));
    console.log(`Telegram chat     ${telegramChatId}`);
